...
export const tagOptions: TagOption[] = _([
  ...
  { value: 'ReligiousConservative', md: 'A channel with a focus on promoting traditional major religion (i.e. Christianity, Judaism, Islam) in the context of politics and culture. Examples: [Ben Shapiro](https://www.youtube.com/channel/UCnQC_G5Xsjhp9fEJKuIcrSw), [PragerU](https://www.youtube.com/user/PragerUniversity)' },
  ...
  { value: 'OrganizedReligion', md: 'Channels that display content from organized religious services/ceremonies or officially endorsed by a religious institution. To be relevant to this dataset, the services should have moralizing/political/salient-sense-making sermons. Examples: [GreekOrthodoxChurch](https://www.youtube.com/channel/UCTVCPFDKwtfTtPzIna-Lucg)' }
...
export const isChannelId = (s: string) => new RegExp(`^([\\w-]{24}|BitChute\\|[\\w]+|https://rumble\\.com/(?:user|c)/[\\w-]+)$`, 'mg').test(s)

export const loadChannelOptions = async (esCfg: EsCfg, s: string): Promise<ChannelOption[]> => {
  if (s == null) return []
  const isChan = isChannelId(s)
  const searchRes = await channelSearch(esCfg, `${isChan ? 'channel_id' : 'channel_title'}:${s}`)
