...
namespace YtReader.Store {
  public class StoreUpgrader {
    ...
    public async Task UpgrateStoreIfNeeded() {
      async Task Run(Task<int> upgrade, string desc) {
        ...
        var upgradeMethods = GetType().GetMethods()
          ...
          .Select(m => (m.m.Name, Upgrade: new Func<Task>(() => (Task) m.m.Invoke(this, new object[] { })), Version: SemVersion.Parse(m.a.Version)));
        ...
        if (md.Version < new SemVersion(1, 1)) {
          var ytStore = new YtStore(Store, Log);
          await ytStore.Captions.Optimise(Log);
        }
    ...
    [Upgrade("0.1")] async Task AddVersionFile() { }
