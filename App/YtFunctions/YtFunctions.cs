...
namespace YtFunctions {
    public static class YtFunctions {
        ...
        public static async Task<string> CollectData([ActivityTrigger] string instanceId, IMSLogger funcLogger) {
            var s = await Init(funcLogger);
            s.Log.Information("About to collect data");

            IContainerGroup g;

            try {
                g = await YtCollectRunner.Start(s.Log, s.Cfg);
            } catch (Exception ex) {
                s.Log.Error("Error collecting data {Error}", ex.Message, ex);
                throw;
            }
            ...
            s.Log.Information("Started collecting data '{ProvisionState}' '{State}': {LogContent}", g.ProvisioningState, g.State, log);
        ...
        [FunctionName("UpdateAllChannels_Timer")]
        public static async Task Run([TimerTrigger("0 0 21 * * *")]TimerInfo myTimer, [OrchestrationClient] DurableOrchestrationClient starter, IMSLogger log) {
            var instanceId = await starter.StartNewAsync("UpdateAllChannels_Orchestration", "");
        }
    }
}