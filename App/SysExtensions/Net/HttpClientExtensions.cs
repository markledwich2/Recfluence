...
namespace SysExtensions.Net
{
    public static class HttpClientExtensions
    {
        ...
        public static string FormatCompact(this HttpRequestMessage url)
        {
            return $"{url.Method} {url.RequestUri}";
        }
        ...
        /// </summary>
        public static async Task<JsonReader> ContentAsJsonReader(this HttpResponseMessage response) => new JsonTextReader(await response.ContentAsStream()) { CloseInput = true };
            ...
            HttpCompletionOption completion, ILogger log)
        {
            ...
            log?.Verbose("{Method} {Server}{Path} {Status} in {Duration}. {ErrorMessage}",
                request.Method, FormatHostPart(request.RequestUri), request.RequestUri.PathAndQuery, response.StatusCode, timer.Elapsed.Humanize(2), errorContent);
