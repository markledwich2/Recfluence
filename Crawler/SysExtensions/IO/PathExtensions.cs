...
namespace SysExtensions.IO
{
    public static class PathExtensions
    {
        ...
        public static async Task<byte[]> ReadBytesAsync(this FPath path)
        {
            ...
            using(var sr = path.Open(SystemIO.FileMode.Open, SystemIO.FileAccess.Read))
            {
                await sr.CopyToAsync(mem);
            }
            return mem.ToArray();
        }

        public static SystemIO.StreamReader OpenText(this FPath path)
        {
            return SystemIO.File.OpenText(path.FullPath);
        }
        ...
        {
            return SystemIO.File.Open(path.ToString(), mode, access, share);
        }
        ...
        /// <summary>
        ///     Returns an absoute path with the given one inside the app data folder with the given app name
        /// </summary>
        ...
        public static FPath InAppData(this FPath relativePath, string appName)
        {
            ...
            return new FPath(Environment.GetEnvironmentVariable("LocalAppData")).Combine(appName).Combine(relativePath);
        }


        public static FPath PathWithoutExtension(this FPath pathToMsql)
        {
            return pathToMsql.Parent().Combine(pathToMsql.FileNameWithoutExtension);
        }
